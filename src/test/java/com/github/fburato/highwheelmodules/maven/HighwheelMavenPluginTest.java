...
public class HighwheelMavenPluginTest {
    ...
    public void shouldSucceedOnStrictBaseSpecification() {
        ...
        assertThat(logFile).matches("(?s).*Facade --> fanIn:     2, fanOut:     3.*")
    ...
    public void shouldFailOnNonExistingSpec() {
        ...
        assertThat(logFile).matches("(?s).*Cannot read from specification file 'not-a-spec'.*");
    }
    ...
    public void shouldFailOnWrongSpec() {
        ...
        final String logFile = runPluginAndReturnLog(true);
        assertThat(logFile).matches("(?s).*The following dependencies violate the specification:.*")
    ...
    public void shouldRunMultipleAnalysisInOnePassSuccessfully() {
        ...
        assertThat(logFile).matches("(?s).*Starting strict analysis on.*spec.hwm'.*"
                ...
                + "Analysis on .*spec.hwm.* complete.*" + "Starting strict analysis on .*spec-with-prefix.hwm.*"
                + "No dependency violation detected.*" + "No direct dependency violation detected.*"
    ...
    public void shouldAcceptSpecificationWithPrefix() {
        ...
        final String logFile = runPluginAndReturnLog(false);
        assertThat(logFile).matches("(?s).*No dependency violation detected.*")
                .matches("(?s).*No direct dependency violation detected.*");
    }
    ...
    public void shouldReportEvidenceInCaseOfFailure() {
        ...
        assertThat(logFile).matches(
                "(?s).*Facade -> Utils. Expected path: \\(empty\\), Actual module path: Facade -> CoreInternals -> Utils.*"
                        + "Actual evidence paths:.*" + "Facade -> CoreInternals:.*"
